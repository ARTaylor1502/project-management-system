<template>
    <modal @close-modal="closeModal">
        <template slot="header">Update User Address</template>
        <template slot="body">
            <form v-if="existing_address" @submit.prevent="updateUserAddress(user_id, existing_address.id, existing_address)">
                <div class="form-group">
                    <label class="col-form-label" for="address_name">Name</label>
                    <input class="form-control" type="text" id="address_name" name="address_name" v-model="existing_address.address_name"/>
...
<script>
    ...
    export default {
        name: "edit-user-address-modal",
        components: {
            modal
        },
        mixins: [AddressHelper],
        props: ['existing_address', 'user_id'],
        data() {
            return {
                errors: {}
            }
        ...
        methods: {
            closeModal() {
                this.$emit('edit-user-address-modal-closed');
            }
        }
    }
</script>
